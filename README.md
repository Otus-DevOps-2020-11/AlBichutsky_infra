...
Удалим каталог `~/appuser` и повторно запустим `playbook`. `ansible` проверит, что репозиторий отсутствует и выполнит его клонирование. Изменения отобразятся при выводе: `changed=1`
...
Приложен bash-скрипт `dynamic_inv.sh` - формирует список хостов для Ansible динамически во время выполнения.  
IP-адреса инстансов в скрипте определяются в соответствии с их названиями в YaCloud, заданными ранее через `terraform`: 
- `reddit-app-0`   
- `reddit-db-dev1`
...
```
Файл динамического инвентори `inventory.json` сгенерировал командой и приложил к ДЗ (формально он не используется)

```bash
./dynamic_inv.sh --list > inventory.json
```

При запуске Ansible вызываем скрипт динамического инвентори с помощью ключей `-i` или `--inventory`

```bash
# проверить доступность хостов в группе app
...
Скрипт добавил в `ansible.cfg`, чтобы была возможность использовать динамический инвентори без явного указания файла.
...
Проверим доступность всех хостов, указанных в статическом и динамическом инвентори командой `ansible all -m ping`
